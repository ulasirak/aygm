...
export default function Footer() {
  ...
  return (
    <footer style={{ background: "#0A5C58", color: "rgba(255,255,255,0.65)" }}>
      <div style={{ height: 3, background: "linear-gradient(90deg, #0B7C77 0%, #6FDDD6 40%, var(--teal-light) 70%, transparent 100%)" }} />
        ...
        <div className="footer-grid" style={{ display: "grid", gap: "2rem", alignItems: "start" }}>
          ...
          <div>
            <Link href="/" style={{ display: "flex", alignItems: "center", gap: "0.875rem", marginBottom: "1.25rem", textDecoration: "none", width: "fit-content" }}>
              <div style={{ width: 44, height: 44, borderRadius: "0.875rem", background: "rgba(111,221,214,0.1)", border: "1px solid rgba(111,221,214,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <FaTrain style={{ color: "#6FDDD6", fontSize: 17 }} />
              </div>
              <div>
                ...
                <div style={{ fontSize: "0.58rem", color: "#6FDDD6", letterSpacing: "0.15em", textTransform: "uppercase", marginTop: "0.15rem" }}>{t("footer.aygm_label")}</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
              {[
                ...
                <Link key={text} href={href} target={external ? "_blank" : undefined} rel={external ? "noopener noreferrer" : undefined} style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem", textDecoration: "none" }} className="group">
                  <div style={{ width: 30, height: 30, borderRadius: "0.5rem", background: "rgba(111,221,214,0.07)", border: "1px solid rgba(111,221,214,0.12)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <Icon style={{ color: "#6FDDD6", fontSize: 10, opacity: 0.7 }} />
                  </div>
          ...
          {footerLinks.map((section) => (
            <div key={section.title}>
              <h4 style={{ fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#6FDDD6", marginBottom: "1.5rem" }}>
                {section.title}
              </h4>
          ...
          <div>
            <h4 style={{ fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#6FDDD6", opacity: 0.6, marginBottom: "1.5rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              {[
                ...
                <Link key={href} href={href} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.75rem 1rem", borderRadius: "0.75rem", background: "rgba(111,221,214,0.04)", border: "1px solid rgba(111,221,214,0.1)", textDecoration: "none", fontSize: "0.8rem", color: "rgba(255,255,255,0.6)" }} className="hover:bg-white/10">
                  <span>{label}</span>
      ...
      <div style={{ borderTop: "1px solid rgba(111,221,214,0.08)", background: "rgba(0,0,0,0.25)" }}>
