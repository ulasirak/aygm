...
export default function TimelineSection() {
  ...
  const statusIcon: Record<TimelineStatus, ReactElement> = {
    done: <FaCheckCircle style={{ color: "#0B7C77", fontSize: 16 }} />,
  ...
  const statusLabel: Record<TimelineStatus, { text: string; color: string; bg: string }> = {
    done:     { text: t("timeline.done"),   color: "#0B7C77",       bg: "rgba(11,124,119,0.1)" },
  ...
  return (
    <section className="section-padding" style={{ background: "linear-gradient(to bottom, white 0%, white 65%, #E9F7F6 100%)" }}>
      <div className="container-aygm">
        ...
        <div className="max-w-3xl mx-auto">
          <div className="relative">
            <div className="absolute left-5 top-6 bottom-6 w-0.5" style={{ background: "linear-gradient(to bottom, #6FDDD6, #0B7C77, var(--teal), var(--gray-200))" }} />
            <div className="space-y-10">
              {timeline.map((item, i) => {
                const sl = statusLabel[item.status];
                return (
                  <div key={i} className="relative flex gap-6">
                    <div className="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center z-10" style={{ background: item.status === "active" ? "rgba(201,168,76,0.1)" : item.status === "done" ? "rgba(11,124,119,0.1)" : "var(--gray-100)", border: item.status === "active" ? "2px solid var(--gold)" : item.status === "done" ? "2px solid #0B7C77" : "2px solid var(--gray-200)" }}>
                      {statusIcon[item.status]}
                    </div>
                    <div className="flex-1 rounded-xl p-7" style={{ background: item.status === "active" ? "linear-gradient(135deg, rgba(201,168,76,0.06) 0%, rgba(11,124,119,0.04) 100%)" : item.status === "done" ? "rgba(11,124,119,0.04)" : "transparent", border: item.status === "active" ? "1px solid rgba(201,168,76,0.2)" : item.status === "done" ? "1px solid rgba(11,124,119,0.12)" : "1px solid transparent" }}>
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        <span className="text-xs font-bold tabular-nums" style={{ color: "#0B7C77", fontFamily: "var(--font-body)" }}>
                      ...
                      <h3 className="font-bold text-lg" style={{ color: "var(--teal-dark)", fontFamily: "var(--font-heading)", marginBottom: "1.25rem" }}>
                        {t(item.titleKey)}
                      </h3>
